...
@onready var item_type_select: OptionButton = %ItemTypeSelect
@onready var amount_text: LineEdit = %AmountText


func _ready() -> void:
	for item_type in Types.Item.values():
		var item_icon := Items.get_info(item_type).icon
		item_type_select.add_icon_item(item_icon, "")
	amount = 1


func _update_amount_text() -> void:
	# try and preserve the caret position when updating the amount
	var old_caret_column := amount_text.caret_column
	amount_text.text = str(amount)
	amount_text.caret_column = old_caret_column
...
func _on_add_items_button_pressed() -> void:
	var item_type := item_type_select.selected as Types.Item
